...
def save_training_history(history: list[dict], output_dir: Path):
    history_path = output_dir / "simple_cnn_training_history.csv"
    ...
    plt.xlabel("Epoca")
    plt.ylabel("Loss")
    plt.title("Historico de perda - CNN simples")
    plt.legend()
    plt.tight_layout()
    plt.savefig(output_dir / "simple_cnn_loss_history.png", dpi=150)
    plt.close()
    ...
    plt.xlabel("Epoca")
    plt.ylabel("Metrica")
    plt.title("Historico de metricas - CNN simples")
    plt.legend()
    plt.tight_layout()
    plt.savefig(output_dir / "simple_cnn_metrics_history.png", dpi=150)
    plt.close()
...
def save_run_config(args, output_dir: Path, class_names: list[str], device: torch.device):
    ...
    with (output_dir / "simple_cnn_run_config.json").open("w", encoding="utf-8") as file:
        json.dump(config, file, indent=2)


def main():
    parser = argparse.ArgumentParser(description="Treina uma CNN simples do zero.")
    ...
    best_val_f1 = -1.0
    best_checkpoint = output_dir / "simple_cnn_best.pth"
    history = []
    ...
    save_metrics(test_metrics, output_dir / "simple_cnn_metrics.json")
    plot_confusion_matrix(
        test_true,
        test_pred,
        datasets_by_split["test"].classes,
        output_dir / "simple_cnn_confusion_matrix.png",
    )
